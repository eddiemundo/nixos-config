    ...
    my-hs-pkgs = super.haskell.packages.${hs-compiler}.override {
      overrides = self: super:
        let
          ...
          json-autotype-src = pkgs.fetchFromGithub {
            owner  = "mgajda";
            repo   = "json-autotype";
            rev    = "c59c09272871f69fea21f5fc32ea6d41fd09c93f";
            sha256 = "06iklj51d9kh9bhc42lrayypcpgkjrjvna59w920ln41rskhjr4y";
          };
        in {
          mkDerivation = disableOptionalHaskellBuildSteps super;

          haskell-language-server = pkgs.haskell.lib.justStaticExecutables (self.callCabal2nix "haskell-language-server" hls-src {});

          ghcide = self.callCabal2nix "ghcide" ghcide-src {};

          brittany = self.callCabal2nix "brittany" brittany-src {};

          json-autotype = self.callCabal2nix "json-autotype" json-autotype-src {};
          ...
          cabal-install = pkgs.callPackage ./cabal-3.2.0.0.nix { inherit pkgs; };
        };
        ## For ghcide and GHC 8.8.2
...
in
  pkgs.mkShell {
    buildInputs = [ 
      hs.compiler.${hs-compiler}
      pkgs.my-hs-pkgs.cabal-install
      hs-pkgs.haskell-language-server
      hs-pkgs.json-autotype
      hs-pkgs.cabal-fmt
      pkgs.zlib ];
    #pkgs.my-hs-pkgs.haskell-language-server
  }
